/*
 * File: map.h
 * Last modified on Wed Apr  1 07:50:43 2009 by eroberts
...
template <typename ValueType>
  class Map {
	public:
		 ...
		 * associated value from the map using get or operator[].
		 */
		Iterator iterator();
		...
		class Iterator {
			...
			private:
				/* This section should be ignored by clients of map.h */
				Iterator(Map *mp);
				Map *mp;
				int bucketIndex;
				void *cellPtr;
 ...
 * map should provide O(1) performance on the put/remove/get operations.
 */
...
template <typename ValueType>
  Map<ValueType>::Map(int sizeHint)
	{
		if (sizeHint < 0) cout<<"Negative sizeHint given to Map constructor\n";
  ...
  ValueType Map<ValueType>::get(string key)
	{
		if (containsKey(key))
			return (*this)[key];
		//cout<<"Attempt to get value for key which is not contained in map.";
	...
	void Map<ValueType>::mapAll(void (*fn)(string key, ValueType value, ClientData &cd), ClientData &data)
	{
		...
		for (int i = 0 ; i < buckets.size(); i++) {
			for (cell *cp = buckets[i]; cp != NULL; cp = cp->next) {
				...
				if (t0 != timestamp) {
					cout<<"mapAll: Map structure changed";
				}
			}
		}
	}
  ...
  void Map<ValueType>::expandAndRehash()
	{
	    ...
	    for (int i = 0; i < oldBuckets.size(); i++)
	        for (cell *cp = oldBuckets[i]; cp != NULL; cp = cp->next)
	            put(cp->key, cp->value);	// add all old cells into new buckets
...
template <typename ValueType>
  bool Map<ValueType>::Iterator::hasNext()
	{
		if (mp == NULL) cout<<"hasNext called on uninitialized iterator";
		if (timestamp != mp->timestamp) {
			cout<<"Map structure has been modified";
		}
		return cellPtr != NULL;
	}
...
template <typename ValueType>
  string Map<ValueType>::Iterator::next()
	{
		if (mp == NULL) cout<<"next called on uninitialized iterator";
		if (!hasNext()) cout<<"Attempt to get next from iterator where hasNext() is false";
		string result = ((cell *) cellPtr)->key;
		advanceToNextKey();
		return result;
	}
